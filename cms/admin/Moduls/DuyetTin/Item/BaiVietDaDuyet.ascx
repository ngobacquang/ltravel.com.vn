<%@ Control Language="C#" AutoEventWireup="true" CodeFile="BaiVietDaDuyet.ascx.cs" Inherits="cms_admin_Moduls_DuyetTin_Item_BaiVietDaDuyet" %>
...
<div id="admitem">
  ...
  <div class="BgTabTitle box-post" align="center">
    ...
    <div class="split">|</div>
    <div class="cot3">
      <asp:LinkButton ID="lbtDate" runat="server" OnClick="lbtDate_Click" CssClass="arrowSort" ToolTip="Click để sắp xếp danh sách theo trường này"><%=Developer.DuyetTinKeyword.NgayDuyet %></asp:LinkButton>
    </div>
    <div class="split">|</div>
    <div class="cot3"><%=Developer.DuyetTinKeyword.NguoiDang %></div>
    <div class="split">|</div>
    <div class="cot3"><%=Developer.DuyetTinKeyword.NguoiDuyet %></div>
    <div class="split">|</div>
    <div class="cot5">Trạng thái</div>
    <div class="split">|</div>
      ...
      <ItemTemplate>
        <div class="Item" id="Item-<%#Eval("IID").ToString()%>">
          <div class="bgItem box-post">
            ...
            <div class="cot2" align="left">
              <div class="fl">
                <%#ImagesExtension.GetImage(GetPicByApp(Eval("VIAPP").ToString()), Eval("VIIMAGE").ToString(), "", "SizeImage", true, true, Eval("VICONTENT").ToString())%>
              </div>
              <div>
            ...
            <div class="split">|</div>
            <div class="cot3"><%#LayInfoNguoiDang(StringExtension.LayChuoi(Eval("VISEOMETACANONICAL").ToString(), "", 1)) %></div>
            <div class="split">|</div>
            <div class="cot5" align="center">
              <a id="nc<%#Eval("IID").ToString()%>" title="<%#Eval("VIAPP").ToString() == "ADV" ? Developer.DuyetTinKeyword.GoBoQuangCao : Developer.DuyetTinKeyword.GoBoBaiViet %>" href="javascript:UpdateEnableItem<%#Eval("VIAPP").ToString() == "ADV" ? "Adv" : "New" %>('GoBo<%#Eval("VIAPP").ToString() == "ADV" ? "QuangCao" : "BaiViet" %>', <%#Eval("IID").ToString()%>, '1', '', '<%#Eval("VIAPP") %>')" class="EnableIcon1">&nbsp;</a>
            </div>
            <div class="split">|</div>

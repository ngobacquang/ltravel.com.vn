...
public partial class cms_admin_Moduls_Hotel_Cate_ControlCate : System.Web.UI.UserControl
{
    ...
    public string DisplayCate(DataTable dt)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox box-stt\" onchange=\"UpdateOrderCate_Hotel('" + dt.Rows[i]["IGID"].ToString() + "','0')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
            ...
            s += "<a style='margin-right:7px' title='Click để xem và thêm mới " + HotelKeyword.Hotel2 + " cho danh mục này' href=\"" + UrlExtension.WebisteUrl + "admin.aspx?uc=" + app + "&igid=" + dt.Rows[i]["IGID"] + "&suc=" + TypePage.Item + "\"><span class='iconItem plus'><!----></span></a>";
            s += "<a style='margin-right:5px' title='Click để chỉnh sửa danh mục này' href=\"" + link + "\"><span class='iconEdit'><!----></span></a>";

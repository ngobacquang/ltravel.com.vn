...
public partial class cms_admin_Moduls_Tour_GroupItem_ControlGroupItem : System.Web.UI.UserControl
{
    ...
    public string DisplayCate(DataTable dt)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox box-stt\" onchange=\"UpdateOrderGroupItem_Tour('" + dt.Rows[i]["IGID"].ToString() + "','0')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
            ...
            s += "<a style='margin-right: 5px' href=\"javascript:void(0)\" onclick=\"NewWindow_('" + LinkAddItemToGroup(dt.Rows[i]["IGID"].ToString(), dt.Rows[i]["IGPARENTSID"].ToString(), dt.Rows[i]["VGNAME"].ToString()) + "','ImageList','800','450','yes','yes')\"><span class=\"ImageFolderAddRecord\"><!----></span></a>";
            s += "<a style='margin-right: 5px' href=\"" + link + "\"><span class='iconEdit'><!----></span></a>";
    ...
    public string DisplaySubCate(DataTable dt, string igparentid)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s += "<div class=\"cot1 box-cb\"><input id=\"CbGroup_" + igidParent + "_" + valueCb + dt.Rows[i]["IGID"].ToString() + "\" type=\"checkbox\" onclick=\"CheckAllCheckBox('CbGroup_" + igidParent + "_" + valueCb + dt.Rows[i]["IGID"].ToString() + "',this)\" /></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot2\" align=\"left\">";
            if (!CountChild.Equals("0"))
            {
            ...
            s += "<div class=\"cot4\" align=\"center\">" + SetPosition(dt.Rows[i]["VGPARAMS"].ToString()) + "</div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot5\" align=\"center\"><input id=\"TbOrder" + dt.Rows[i]["IGID"].ToString() + "\" type=\"text\" value=\"" + dt.Rows[i]["IGORDER"].ToString() + "\" class=\"TextInBox box-stt\" onchange=\"UpdateOrderGroupItem_Tour('" + dt.Rows[i]["IGID"].ToString() + "','" + igparentid + "')\" /></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot6\" align=\"center\"><a id=\"nc" + dt.Rows[i]["IGID"].ToString() + "\" href=\"javascript:UpdateEnableGroup('" + dt.Rows[i]["IGID"].ToString() + "')\" class=\"EnableIcon" + dt.Rows[i]["IGENABLE"].ToString() + "\">&nbsp;</a></div>";
            s += "<div class=\"splitc\">|</div>";
            s += "<div class=\"cot7 box-cc\">";
